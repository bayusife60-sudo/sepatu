    ...
    <div class="container-fluid">
        ...
        <div class="row mb-5 align-items-end">
            <div class="col-md-8">
                <span class="text-primary text-uppercase"
                    style="letter-spacing: 2px; font-size: 0.85rem; font-weight: 500;">{{ Auth::user()->role == 'owner' ? 'Owner Area' : 'Admin Area' }}</span>
                <h2 class="mb-0 mt-2" style="font-family: 'Playfair Display', serif; font-size: 2.5rem;">Dashboard
                    Operasional</h2>
                <p class="text-muted mt-2 mb-0" style="font-weight: 300;">Ringkasan aktivitas hari ini,
                    {{ now()->translatedFormat('d F Y') }}</p>
            </div>
            <div class="col-md-4 text-md-end mt-4 mt-md-0">
                <a href="{{ route('admin.orders.create') }}" class="btn btn-primary" style="padding: 0.6rem 1.5rem;"><i
                        class="fas fa-plus me-2"></i>Order Baru</a>
            </div>
        </div>
        ...
        <div class="row mb-5 g-4">
            ...
            <div class="col-md-4">
                <div class="card h-100"
                    style="background: linear-gradient(145deg, var(--secondary-light) 0%, #1a1a1a 100%); border-top: 3px solid var(--primary);">
                    <div class="card-body p-4 position-relative overflow-hidden">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h6 class="text-uppercase text-muted mb-0" style="letter-spacing: 1px; font-size: 0.8rem;">Order
                                Aktif</h6>
                        ...
                        <h2 class="display-5 fw-bold mb-1" style="font-family: 'Playfair Display', serif;">
                            {{ $activeOrders }}</h2>
                        <p class="text-muted small mb-0"><span class="text-success"><i
                                    class="fas fa-arrow-up me-1"></i>Dalam proses</span> pengerjaan</p>

                        <i class="fas fa-spinner position-absolute text-white"
                            style="font-size: 10rem; opacity: 0.02; bottom: -20px; right: -20px; transform: rotate(-15deg);"></i>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="card h-100"
                    style="background: linear-gradient(145deg, var(--secondary-light) 0%, #1a1a1a 100%);">
                    <div class="card-body p-4 position-relative overflow-hidden">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h6 class="text-uppercase text-muted mb-0" style="letter-spacing: 1px; font-size: 0.8rem;">Perlu
                                Pickup Hari Ini</h6>
                        ...
                        <i class="fas fa-truck-pickup position-absolute text-white"
                            style="font-size: 10rem; opacity: 0.02; bottom: -20px; right: -20px; transform: rotate(-15deg);"></i>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="card h-100"
                    ...
                    <div class="card-body p-4 position-relative overflow-hidden">
                        ...
                        <i class="fas fa-box-open position-absolute text-white"
                            style="font-size: 10rem; opacity: 0.02; bottom: -20px; right: -20px; transform: rotate(-15deg);"></i>
                    </div>
                </div>
            </div>
        ...
        <div class="row mb-5">
            <div class="col-12">
                <div class="card border-0 shadow-sm" style="background: linear-gradient(145deg, #1a1a1a 0%, #111111 100%);">
                    <div
                        class="card-header border-bottom border-white-5 bg-transparent py-4 px-4 d-flex justify-content-between align-items-center">
                        <div>
                            <h4 class="mb-1" style="font-size: 1.25rem; font-weight: 600;">
                                {{ Auth::user()->role == 'owner' ? 'Performa Keuangan' : 'Volume Pekerjaan' }}</h4>
                            <p class="text-muted small mb-0">
                                {{ Auth::user()->role == 'owner' ? 'Perbandingan Pendapatan vs Pengeluaran' : 'Statistik Jumlah Sepatu Masuk' }}
                            </p>
                        </div>
                        <div class="btn-group" role="group"
                            style="background: rgba(255,255,255,0.05); padding: 4px; border-radius: 12px;">
                            <button type="button" class="btn btn-sm px-3 chart-toggle active" id="toggleDaily"
                                onclick="updateChart('daily')">Harian</button>
                            <button type="button" class="btn btn-sm px-3 chart-toggle" id="toggleMonthly"
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center text-start py-4">
                ...
                <a href="#" class="btn btn-sm btn-outline-custom" style="padding: 0.4rem 1rem; font-size: 0.75rem;">Lihat
                    Semua</a>
            </div>
                    ...
                    <table class="table table-dark table-hover mb-0" style="background-color: transparent;">
                        <thead>
                            <tr style="border-bottom: 2px solid rgba(255,255,255,0.1);">
                                <th class="py-3 px-4 fw-medium text-muted text-uppercase"
                                    style="font-size: 0.8rem; letter-spacing: 1px;">Kode Order</th>
                                <th class="py-3 px-4 fw-medium text-muted text-uppercase"
                                    style="font-size: 0.8rem; letter-spacing: 1px;">Pelanggan</th>
                                <th class="py-3 px-4 fw-medium text-muted text-uppercase"
                                    style="font-size: 0.8rem; letter-spacing: 1px;">Sepatu</th>
                                <th class="py-3 px-4 fw-medium text-muted text-uppercase"
                                    style="font-size: 0.8rem; letter-spacing: 1px;">Status</th>
                                <th class="py-3 px-4 fw-medium text-muted text-uppercase"
                                    style="font-size: 0.8rem; letter-spacing: 1px;">Tanggal</th>
                                <th class="py-3 px-4 fw-medium text-muted text-uppercase text-end"
                                    style="font-size: 0.8rem; letter-spacing: 1px;">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($recentOrders as $order)
                                <tr style="border-bottom: 1px solid rgba(255,255,255,0.05); transition: background-color 0.2s;">
                                    ...
                                    <td class="py-3 px-4 align-middle">
                                        ...
                                        @endphp
                                        <span class="badge {{ $badgeClass }} py-2 px-3 fw-medium rounded-pill"
                                    ...
                                    <td class="py-3 px-4 align-middle text-end">
                                        <a href="{{ route('admin.orders.show', $order) }}"
                                            class="btn btn-sm btn-outline-light rounded-circle d-inline-flex align-items-center justify-content-center"
                                            title="View Detail" style="width: 32px; height: 32px; padding: 0;">
                                            <i class="fas fa-eye" style="font-size: 0.8rem;"></i>
                                        </a>
                                        <a href="{{ route('admin.orders.show', $order) }}"
                                            class="btn btn-sm btn-primary rounded-circle ms-1 d-inline-flex align-items-center justify-content-center"
                                            title="Update Status" style="width: 32px; height: 32px; padding: 0;">
